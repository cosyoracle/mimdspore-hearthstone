...
def visitNode(node):
    ...
    for x in ast.iter_fields(node):
        if str(x[0]) == "ctx":
            continue
        # print(str(x[1]))
        if str(x[0]) == "vararg" or str(x[0]) == "kwarg" or str(x[0]) == 'defaults' or str(
                x[0]) == "decorator_list" or str(x[0]) == "starargs" or str(x[0]) == "kwargs":
            continue
        rootNode = tmpNode
...
for x in sets:
    ...
    for i, y in enumerate(fout):
        ...
        nl = nl.replace("<b>", " <b> ").replace("<i>", " <i> ").replace("</b>", " 在 ").replace("</i>", " 见 ").replace(
            "+", " + ").replace("/", " / ").replace(":", "").replace("在", "</b>").replace(".", " . ").replace("(",
                                                                                                               " ( ").replace(
            ")", " ) ").replace("见", "</i>").replace(";", " ; ").replace(",", " , ").replace("#", " # ").replace("$",
                                                                                                                  " $ ")
        lst = []
        for x in nl.split():
            if "-" in x and x[0] != "-":
